...
- LCOS 尝试节点级局部冲突 objective selection，但直接对齐 high-pass target 导致 Texas/Chameleon 失败，当前只保留 Squirrel 机制线索；
- 当前仍没有可直接包装为 2026 顶会/顶刊主方法的成功 idea；下一代 candidate 必须换机制，优先考虑 downstream separability proxy 或更稳健的 loss reliability，而不是继续调 PCNV/LCOS。
