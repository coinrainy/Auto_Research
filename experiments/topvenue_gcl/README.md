...
当前 active-but-risky candidate：**BPRRNV / Bootstrap-Preserving Selective RRNV**，入口为 `--method bprrnv_gcl`。它不再用 RRNV 替代 Natural-View bootstrap，而是在 `gcn_mlp_gcl` bootstrap loss 之外加入 density/energy selective RR regularizer；默认 `bprrnv_rr_weight=0.1`。初筛显示 0.25 强正则失败，而 0.1 在 Texas/Chameleon/Squirrel/Actor × splits0-2 × seed0 上相对 `gcn_mlp_gcl` 的 mean micro delta 为 +0.027027、+0.003655、+0.000640、+0.005044；整体优于 uniform gate 与 shuffled-pair control。但 Chameleon 上 uniform gate 局部更强，Actor 的 normal/shuffled 差距很小，因此 BPRRNV 只能暂时保留为候选，不能包装成成功主方法。
...
- 当前仍没有可直接包装为 2026 顶会/顶刊主方法的成功 idea；BPRRNV 是最新 active-but-risky candidate，但必须先过更大 split/seed、homophily safety 与 uniform/shuffled/no-density/no-energy controls。RWIRRNV 只保留为 invariance attenuation 机制线索。DPRRNV/NPRRNV 只保留为高密度图机制线索。
