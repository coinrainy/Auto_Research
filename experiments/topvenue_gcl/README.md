...
当前 active-but-risky candidate：**RRNV-GCL / Redundancy-Reduced Natural-View GCL**，入口为 `--method rrnv_gcl`。它用 VICReg/CCA 风格 redundancy reduction 替换 Natural-View BYOL bootstrap，并用 `--rrnv-shuffle-pairs` 做机制 control。Texas 在 splits 0-2 上有强正向信号，Chameleon 小正且 normal-vs-shuffled 基本支持真实配对机制；Actor 仅弱正，Squirrel 均值负向。因此 RRNV 不是最终成功方法，但暂时是当前最值得继续 safety / graph-type adaptation 的候选。
...
- 当前仍没有可直接包装为 2026 顶会/顶刊主方法的成功 idea；RRNV 是当前最值得保留的 active-but-risky candidate。下一代工作应围绕 RRNV 的 Squirrel safety / graph-type adaptation 与强基线同协议复现展开，而不是继续调 DSP、PCNV、LCOS、LCM 或 DARRNV。
