...
- 当前最值得继续推进的是 RAGC-GCL：它不再试图让 GCL embedding 替代 raw features，而是显式保留 raw separability，再检验 learned graph context 的互补增益。RAGC 已通过 10-split raw baseline 扩展，并在 Chameleon/Squirrel 上通过 10-split learned-branch shuffle/random 打假；homophily 上通过 raw-only safety，但 Cora 暴露 fixed concatenation 低于 learned-only 的边界。下一步必须补 Actor/Texas 10-split controls、多 seed、强 baseline paper table，以及协议一致的 raw/learned/RAGC selector。
