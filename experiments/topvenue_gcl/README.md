...
当前 active candidate：**RAGC-GCL / Raw-Anchored Graph Complement GCL**，入口为 `--method ragc_gcl`；强基线 `raw_features` 入口为 `--method raw_features`。RAGC 保留 `gcn_mlp_gcl` 的 Natural-View bootstrap 训练，但最终表示拼接 L2-normalized raw features 与 learned Natural-View embedding，用于验证“图 SSL 是否提供 raw 之外的互补信息”。Actor/Chameleon/Squirrel/Texas × splits0-9 × seed0 × 50 epoch 下，RAGC 相对 `raw_features` 的 F1Mi mean delta 分别为 +0.009408、+0.016886、+0.007397、+0.005405；positive/negative split 数分别为 9/1、9/1、9/1、5/2。10-split learned-branch controls 显示 Actor/Chameleon/Squirrel 严格通过 normal > raw/shuffle/random：Actor normal/raw/shuffle/random F1Mi=0.361118/0.351711/0.337566/0.329605，Chameleon=0.474781/0.457895/0.434211/0.383114，Squirrel=0.338136/0.330740/0.319500/0.299712。Texas normal/raw/shuffle/random=0.813514/0.808108/0.818919/0.718919，说明 Texas 性能仍微正但机制 control 不干净，应作为 WebKB 小图边界而不是主机制证据。Actor seeds1-2 strong-table partial audit 中，RAGC fixed concat F1Mi=0.358618，高于 raw 0.351711 与 learned-only 0.351118；但相对 per-split best(raw, learned-only) 仅 +0.002434，且 7/20 为负。`ragc_auto_gcl` validation selector 与 `ragc_learned_weight=0.5/0.75` 全局缩放均未修复 fixed concat 的 safety 问题。Homophily safety 中，RAGC 相对 raw-only 在 Cora/CiteSeer/PubMed 上分别为 +0.076565、+0.041803、+0.011931 F1Mi，但相对 learned-only `gcn_mlp_gcl` 为 Cora -0.052557、CiteSeer -0.002293、PubMed +0.015835。当前应推进 Complement-Gated RAGC，而不是继续包装 fixed concat。
...
- 当前最值得继续推进的是 RAGC-GCL 的 complement 机制，而不是 fixed concat 形态：它不再试图让 GCL embedding 替代 raw features，而是显式保留 raw separability，再检验 learned graph context 的互补增益。RAGC 已通过 10-split raw baseline 扩展，并在 Actor/Chameleon/Squirrel 上通过 10-split learned-branch shuffle/random 打假；homophily 上通过 raw-only safety，但 Cora 暴露 fixed concatenation 低于 learned-only 的边界，Texas 暴露小图 shuffle control 不干净的边界，Actor seeds1-2 暴露 fixed concat raw-safety 漏洞。下一步必须实现 Complement-Gated RAGC；若不能修复 Actor targeted negatives，应收缩为机制诊断论文路线或放弃 RAGC 主方法。
