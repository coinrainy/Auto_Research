...
因此当前裁决是：放弃“固定双分支 SSPNV = 终稿 SOTA idea”的包装。后续 AFPNV 与 BSPNV 进一步验证了这个判断：简单置信度加权和 branch selection 都没有同时超过 Chameleon semantic-only 与 Squirrel full SSPNV。SSPNV 家族当前降级为机制/消融资产，不再作为 active main idea。
...
新增 `bspnv_gcl`，入口为 `--method bspnv_gcl`。它把 AFPNV 的软置信度加权改成 semantic / spatial / bootstrap 三分支竞争选择：

- semantic score：semantic positives 的 raw propagation signature 相似度；
- spatial score：one-hop spatial positive 的 raw propagation signature 相似度；
- bootstrap score：固定 bias，作为不使用 sampled positive 的保守回退；
- branch temperature：`bspnv_branch_temperature=0.1`；
- bootstrap bias：`bspnv_bootstrap_bias=0.25`。

Chameleon/Squirrel × splits 0-9 × seed0 × 50 epoch 结果：

| Dataset | Variant | ΔF1Mi | ΔF1Ma | Positive/Negative F1Mi | 裁决 |
| --- | --- | ---: | ---: | --- | --- |
| Chameleon | BSPNV | +0.032456 | +0.032602 | 9/1 | 强于 full SSPNV、AFPNV、random semantic，但弱于 semantic-only 与 spatial-only |
| Squirrel | BSPNV | +0.006436 | +0.000592 | 6/4 | 强于 AFPNV/semantic-only/spatial-only/random controls，但弱于 full SSPNV |

BSPNV smoke 诊断显示选择器没有退化为单一分支：
...
裁决：BSPNV 比 AFPNV 更合理，但未达到预设升级门槛。它说明 branch/objective selection 有局部价值，却不能解决 SSPNV 家族的核心问题：Chameleon 上简单 semantic-only 更强，Squirrel 上 full SSPNV 仍更强。因此停止 SSPNV 家族主线，不再继续小调 threshold、temperature 或 branch bias。
...
- BSPNV 未同时超过 Chameleon semantic-only 与 Squirrel full SSPNV；
- 因此 SSPNV / AFPNV / BSPNV 均降级为 ablation assets；
- 暂不继续 homophily safety 或强基线同协议对齐，因为当前方法主张已经不足以支撑顶会主线。

## 下一步

下一步不再继续 SSPNV 家族。建议转向新的机制搜索：

```bash
cd /root/autodl-tmp/Auto_Research/experiments/topvenue_gcl
rg -n "semantic|spatial|high|low|mlp|inference|contrast|positive" ../../third_party_baselines/reference_gcl/S3GCL ../../third_party_baselines/reference_gcl/GraphECL ../../third_party_baselines/reference_gcl/PolyGCL -g '*.py' -g '*.md'
```

下一代 idea 不应再是 SSPNV 的 threshold/temperature 小改，而应回到 strong top-venue reference pattern：MLP inference、filter-aware contrast、结构/语义解耦、以及能明确击败 `gcn_mlp_gcl` strong foundation 的训练目标。
