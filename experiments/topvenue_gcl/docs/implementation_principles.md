...
当前没有通过最终复核的 active main idea。当前 active-but-risky candidate 是 `dsrrnv_gcl`：它保留 RRNV redundancy reduction 训练目标，并用图平均度 gate 在 final representation 中混合 graph/high residual view。Texas 强正、Chameleon 小正，Squirrel 均值由负转正，但 Squirrel shuffled control 更强。`dprrnv_gcl` 证明高密度扰动配对能修复 Squirrel split0，但 Actor/Chameleon full-shuffled 更强；`nprrnv_gcl` 将扰动改为节点级后仍伤害 Chameleon。因此二者只保留为机制线索。后续只能围绕 DS-RRNV 的高密度图机制诊断与强基线同协议对齐推进，不能把它包装成已成功方法。
