...
当前没有通过最终复核的 active main idea。`bprrnv_gcl` 是当前 active-but-risky candidate：它把 RRNV 从主目标降级为 bootstrap-preserving auxiliary regularizer，并用 graph density 与 graph/high energy conflict 控制正则强度。`rwirrnv_gcl` 已从 active-but-risky candidate 降级为机制线索：10 split seed0 中 Texas/Squirrel/Chameleon 有正向性能信号，但 Chameleon 的 constant-weight control 最好、Squirrel 的 shuffled-weight control 最好，说明当前 per-node reliability 排序不能作为主贡献。`eairrnv_gcl` 已验证 graph-level energy attenuation 有局部收益但不能过 Squirrel safety。BPRRNV 的下一步硬门槛是扩展到 splits 0-9 / 多 seed，并继续报告 normal、uniform gate、shuffled pair、no-density 与 no-energy controls。
...
当前保留但未成功的 active candidate：

- `bprrnv_gcl`，Bootstrap-Preserving Selective RRNV。它通过 density/energy selective gate 给 Natural-View bootstrap 增加轻量 RR regularizer；当前只过了 splits0-2 seed0 初筛，尚未过论文级复核。
...
- 当前解释：BPRRNV 比 EAIRRNV/DARRNV 更符合“保留 bootstrap、选择性加入 RR”的方向；Texas 支持最强，Squirrel safety 得到改善，Chameleon 对 uniform gate 仍敏感，Actor 机制差距很弱；
- 当前裁决：BPRRNV 升级为 active-but-risky candidate，但不是成功主方法。下一步必须跑 splits 0-9、至少 seed0/1，补 no-density/no-energy control 与 homophily safety；若 normal 不能稳定优于 uniform/shuffled，立即降级。
