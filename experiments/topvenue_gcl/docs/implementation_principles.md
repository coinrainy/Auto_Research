...
当前没有通过最终复核的 active main idea。当前 active-but-risky candidate 是 `rwirrnv_gcl`：它保留 RRNV redundancy reduction 训练目标和 DS-RRNV final representation，但用节点级 reliability 降低不可靠节点的 invariance 权重。splits 0-2、seed0 中 Texas/Chameleon/Squirrel/Actor 的 normal 均值都不低于 `gcn_mlp_gcl`，其中 Texas 最强；但 Squirrel/Actor 的 shuffled-weight control 差距很小。后续只能围绕 RWIRRNV 的 10 splits / 多 seed 复核、Squirrel/Actor failure analysis 与强基线同协议对齐推进，不能把它包装成已成功方法。
...
- `dprrnv_gcl` 作为最终主方法。
- `nprrnv_gcl` 作为最终主方法。
- `rwirrnv_gcl` 在未通过 10 splits / 多 seed、强基线和 shuffled-weight 机制复核前作为最终主方法。
...
- 当前解释：Texas 是强证据，Chameleon 小正且 control 基本支持，Squirrel/Actor 的性能均值为正但机制 control 很弱；
- RWIRRNV 继续作为 active-but-risky candidate，但进入主线前必须通过 10 splits / 多 seed、强基线和 Squirrel/Actor failure analysis。
