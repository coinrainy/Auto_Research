...
当前没有通过最终复核的 active main idea。当前 active-but-risky candidate 是 `rwirrnv_gcl`：它保留 RRNV redundancy reduction 训练目标和 DS-RRNV final representation，但用节点级 reliability 降低不可靠节点的 invariance 权重。split0 seed0 中 Texas/Chameleon 强正且 reliability 排序 control 支持，Actor 小正但 control 不干净，Squirrel 失败。后续只能围绕 RWIRRNV 的 splits 0-2 复核、Squirrel failure analysis 与强基线同协议对齐推进，不能把它包装成已成功方法。
...
- `dprrnv_gcl` 作为最终主方法。
- `nprrnv_gcl` 作为最终主方法。
- `rwirrnv_gcl` 在未通过 splits 0-2 与 Squirrel 修复前作为最终主方法。
...
- 当前解释：Texas/Chameleon 是强证据，Actor 弱且 control 不干净，Squirrel 是主要失败点；
- RWIRRNV 升级为 active-but-risky candidate，但进入主线前必须通过 splits 0-2 与 Squirrel failure fix。
