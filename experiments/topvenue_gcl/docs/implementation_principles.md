...
当前没有通过最终复核的 active main idea。`rwirrnv_gcl` 已从 active-but-risky candidate 降级为机制线索：10 split seed0 中 Texas/Squirrel/Chameleon 有正向性能信号，但 Chameleon 的 constant-weight control 最好、Squirrel 的 shuffled-weight control 最好，说明当前 per-node reliability 排序不能作为主贡献。`eairrnv_gcl` 已验证 graph-level energy attenuation 有局部收益但不能过 Squirrel safety。下一代候选应转向 bootstrap-preserving 的 density/energy selective RR regularization 或 objective selector，并沿用 normal/shuffled/constant/control sweep 作为硬门槛。
